...
import { Navinecord_USER_AGENT } from "@shared/NavinecordUserAgent";
...
import { serializeErrors, Navinecord_FILES } from "./common";
...
async function githubGet<T = any>(endpoint: string) {
    return fetchJson<T>(API_BASE + endpoint, {
        headers: {
            ...
            "User-Agent": Navinecord_USER_AGENT
        }
    });
}
...
async function fetchUpdates() {
    ...
    data.assets.forEach(({ name, browser_download_url }) => {
        if (Navinecord_FILES.some(s => name.startsWith(s))) {
