            ...
            replacement: [
                {
                    ...
                    replace: "if(Navinecord.Api.Notices.currentNotice)return false;$&"
                },
                {
                    match: /(?<=,NOTICE_DISMISS:function\(\i\){)return null!=(\i)/,
                    replace: (m, notice) => `if(${notice}?.id=="NavinecordNotice")return(${notice}=null,Navinecord.Api.Notices.nextNotice(),true);${m}`
                },
                // FIXME(Bundler agressive inline): Remove the non used compability once enough time has passed
                {
                    ...
                    replace: (m, _, notice) => `if(${notice}?.id=="NavinecordNotice")return(${notice}=null,Navinecord.Api.Notices.nextNotice(),true);${m}`,
                    noWarn: true
                }
            ]
        }
