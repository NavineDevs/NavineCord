...
function makeShortcuts() {
    ...
    return {
        ...
        wpPatcher: { getter: () => Navinecord.WebpackPatcher },
        wpInstances: { getter: () => Navinecord.WebpackPatcher.allWebpackInstances },
        wpsearch: search,
        wpex: extract,
        ...
        PluginsApi: { getter: () => Navinecord.Plugins },
        plugins: { getter: () => Navinecord.Plugins.plugins },
        Settings: { getter: () => Navinecord.Settings },
        Api: { getter: () => Navinecord.Api },
        Util: { getter: () => Navinecord.Util },
            ...
            if (!win.prepared) {
                ...
                [...document.querySelectorAll("style"), ...document.querySelectorAll("link[rel=stylesheet]")].forEach(s => {
                    ...
                    if (s.parentElement?.tagName === "HEAD")
                        doc.head.append(n);
                    else if (n.id?.startsWith("Navinecord-") || n.id?.startsWith("vcd-"))
        ...
        preEnable: (plugin: string) => (Navinecord.Settings.plugins[plugin] ??= { enabled: true }).enabled = true,
...
function loadAndCacheShortcut(key: string, val: any, forceLoad: boolean) {
    ...
    function unwrapProxy(value: any) {
        ...
        } else if (value.$$NavinecordGetWrappedComponent) {
            return forceLoad ? value.$$NavinecordGetWrappedComponent() : value;
        }
...
export default definePlugin({
    ...
    start() {
        ...
        if (!IS_WEB) {
            const Native = NavinecordNative.pluginHelpers.ConsoleShortcuts as PluginNative<typeof import("./native")>;
