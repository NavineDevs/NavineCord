...
function Icon({ isShown }: { isShown: boolean; }) {
    return (
        <svg viewBox="0 0 27 27" width={24} height={24} className="nc-toolbox-icon">
            {isShown
...
function NavinecordPopoutButton({ buttonClass }: { buttonClass: string; }) {
    ...
    return (
        ...
        >
            {(_, { isShown }) => (
                <HeaderBarIcon
                    ref={buttonRef}
                    className={`nc-toolbox-btn ${buttonClass}`}
                    onClick={() => setShow(v => !v)}
                    tooltip={isShown ? null : "Navinecord Toolbox"}
...
export default definePlugin({
    name: "NavinecordToolbox",
    description: "Adds a button to the titlebar that houses Navinecord quick actions",
    ...
    TrailingWrapper({ children, className }: PropsWithChildren<{ className: string; }>) {
        return (
            <>
                {children}
                <ErrorBoundary noop>
                    <NavinecordPopoutButton buttonClass={className} />
                </ErrorBoundary>
            </>
        );
    },
