...
export async function checkForUpdates() {
    changes = await Unwrap(NavinecordNative.updater.getUpdates());
...
export async function update() {
    ...
    const res = await Unwrap(NavinecordNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(NavinecordNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(NavinecordNative.updater.getRepo());
