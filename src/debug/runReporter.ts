...
async function runReporter() {
    ...
    try {
        ...
        addPatch({
            ...
            replacement: {
                match: /(?<="use strict";)/,
                replace: "Navinecord.Webpack._initReporter();"
            }
        }, "Navinecord Reporter");

        // @ts-expect-error
        Navinecord.Webpack._initReporter = function () {
        ...
        for (const [searchType, args] of Webpack.lazyWebpackSearchHistory) {
            ...
            try {
                ...
                if (result == null || (result.$$NavinecordGetWrappedComponent != null && result.$$NavinecordGetWrappedComponent() == null)) throw new Error("Webpack Find Fail");
            } catch (e) {
                let logMessage = searchType;
                if (method === "find" || method === "proxyLazyWebpack" || method === "LazyComponentWebpack") {
                    if (args[0].$$NavinecordProps != null) {
                        logMessage += `(${args[0].$$NavinecordProps.map(arg => `"${arg}"`).join(", ")})`;
                    } else {
...
// Run after the Navinecord object has been created.
// We need to add extra properties to it, and it is only created after all of Navinecord code has ran
