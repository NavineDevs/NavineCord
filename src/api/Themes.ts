...
import { ThemeStore } from "@Navinecord/discord-types";
import { PopoutWindowStore } from "@webpack/common";

import { userStyleRootNode, NavinecordRootNode } from "./Styles";
...
async function toggle(isEnabled: boolean) {
    if (!style) {
        if (isEnabled) {
            style = createAndAppendStyle("Navinecord-custom-css", userStyleRootNode);
            NavinecordNative.quickCss.addChangeListener(css => {
                style.textContent = css;
                // At the time of writing this, changing textContent resets the disabled state
                style.disabled = !Settings.useQuickCss;
                updatePopoutWindows();
            });
            style.textContent = await NavinecordNative.quickCss.get();
        }
    } else
...
async function initThemes() {
    themesStyle ??= createAndAppendStyle("Navinecord-themes", userStyleRootNode);
    ...
    if (IS_WEB) {
        for (const theme of enabledThemes) {
            const themeData = await NavinecordNative.themes.getThemeData(theme);
            if (!themeData) continue;
            const blob = new Blob([themeData], { type: "text/css" });
            links.push(URL.createObjectURL(blob));
        }
    } else {
        const localThemes = enabledThemes.map(theme => `Navinecord:///themes/${theme}?v=${Date.now()}`);
...
function applyToPopout(popoutWindow: Window | undefined, key: string) {
    ...
    doc.querySelector("Navinecord-root")?.remove();

    doc.documentElement.appendChild(NavinecordRootNode.cloneNode(true));
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!IS_WEB) {
        NavinecordNative.quickCss.addThemeChangeListener(initThemes);
    }
}, { once: true });
