...
import { CommandArgument, CommandContext, CommandOption } from "@Navinecord/discord-types";

import { sendBotMessage } from "./commandHelpers";
import { ApplicationCommandInputType, ApplicationCommandOptionType, ApplicationCommandType, NavinecordCommand } from "./types";

export * from "./commandHelpers";
export * from "./types";

export let BUILT_IN: NavinecordCommand[];
export const commands = {} as Record<string, NavinecordCommand>;
...
export const _init = function (cmds: NavinecordCommand[]) {
    try {
        BUILT_IN = cmds;
...
export const _handleCommand = function (cmd: NavinecordCommand, args: CommandArgument[], ctx: CommandContext) {
    if (!cmd.isNavinecordCommand)
 ...
 * @param opt
 */
export function prepareOption<O extends CommandOption | NavinecordCommand>(opt: O): O {
...
// investigate
function registerSubCommands(cmd: NavinecordCommand, plugin: string) {
...
export function registerCommand<C extends NavinecordCommand>(command: C, plugin: string) {
    if (!BUILT_IN) {
        console.warn(
            "[CommandsAPI]",
    ...
    command.isNavinecordCommand = true;
