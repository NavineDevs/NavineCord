...
export const styleMap = window.NavinecordStyles ??= new Map();

export const NavinecordRootNode = document.createElement("Navinecord-root");
/**
 * Houses all Navinecord core styles. This includes all imported css files
 */
export const coreStyleRootNode = document.createElement("Navinecord-styles");
/**
 * Houses all plugin specific managed styles
 */
export const managedStyleRootNode = document.createElement("Navinecord-managed-styles");
/**
 * Houses the user's themes and quick css
 */
export const userStyleRootNode = document.createElement("Navinecord-user-styles");

NavinecordRootNode.style.display = "none";
NavinecordRootNode.append(coreStyleRootNode, managedStyleRootNode, userStyleRootNode);

export function initStyles() {
    const osValuesNode = createAndAppendStyle("Navinecord-os-theme-values", coreStyleRootNode);
    createAndAppendStyle("Navinecord-text", coreStyleRootNode).textContent = generateTextCss();
    const rendererCssNode = createAndAppendStyle("Navinecord-css-core", coreStyleRootNode);
    const vesktopCssNode = IS_VESKTOP ? createAndAppendStyle("vesktop-css-core", coreStyleRootNode) : null;
    createAndAppendStyle("Navinecord-margins", coreStyleRootNode).textContent = generateMarginCss();

    NavinecordNative.native.getRendererCss().then(css => rendererCssNode.textContent = css);
    if (IS_DEV) {
        NavinecordNative.native.onRendererCssUpdate(newCss => {
    ...
    NavinecordNative.themes.getSystemValues().then(values => {
...
document.addEventListener("DOMContentLoaded", () => {
    document.documentElement.append(NavinecordRootNode);
}, { once: true });
...
export function enableStyle(name: string) {
    ...
    if (!style.dom) {
        ...
        style.dom.dataset.NavinecordName = style.name;
    }
    compileStyle(style);
