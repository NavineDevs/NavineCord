...
export async function checkCloudUrlCsp() {
    ...
    if (host === "api.Navinecord.dev") return true;

    if (await NavinecordNative.csp.isDomainAllowed(Settings.cloud.url, ["connect-src"])) {
        return true;
    }

    const res = await NavinecordNative.csp.requestAddOverride(Settings.cloud.url, ["connect-src"], "Cloud Sync");
    if (res === "ok") {
        Alerts.show({
...
export async function getAuthorization() {
    const secrets = await DataStore.get<Record<string, string>>("Navinecord_cloudSecret") ?? {};

    const origin = getCloudUrlOrigin();

    // we need to migrate from the old format here
    if (secrets[origin]) {
        await DataStore.update<Record<string, string>>("Navinecord_cloudSecret", secrets => {
            secrets ??= {};
