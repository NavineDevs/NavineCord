...
async function syncSettings() {
    if (localStorage.Navinecord_cloudSyncDirection === undefined) {
        // by default, sync bi-directionally
        localStorage.Navinecord_cloudSyncDirection = "both";
    }

    // pre-check for local shared settings
    if (
        Settings.cloud.authenticated &&
        !await dsGet("Navinecord_cloudSecret") // this has been enabled due to local settings share or some other bug
    ) {
        ...
        showNotification({
            ...
            onClick: () => SettingsRouter.open("NavinecordCloud")
        });
        return;
    }
    ...
    if (
        ...
        localStorage.Navinecord_cloudSyncDirection !== "manual" // if we're not in manual mode
    ) {
        if (localStorage.Navinecord_settingsDirty && shouldCloudSync("push")) {
    ...
    SettingsStore.addGlobalChangeListener(() => {
        localStorage.Navinecord_settingsDirty = true;
    ...
    try {
        ...
        if (Settings.autoUpdate) {
            ...
            if (Settings.autoUpdateNotification) {
                notify({
                    title: "Navinecord has been updated!",
        ...
        notify({
            title: "A Navinecord update is available!",
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Navinecord.",
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (IS_DISCORD_DESKTOP && Settings.winNativeTitleBar && IS_WINDOWS) {
        createAndAppendStyle("Navinecord-native-titlebar-style", coreStyleRootNode).textContent = "[class*=titleBar]{display: none!important}";
    }
}, { once: true });
