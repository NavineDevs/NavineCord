...
// Only import pure modules that don't import other parts of Navinecord.
...
const setCssDebounced = debounce((css: string) => NavinecordNative.quickCss.set(css));

const themeStore = DataStore.createStore("NavinecordThemes", "NavinecordThemeData");

// probably should make this less cursed at some point
window.NavinecordNative = {
    themes: {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Vendicated/Navinecord" }),
    ...
    quickCss: {
        get: () => DataStore.get("NavinecordQuickCss").then(s => s ?? ""),
        set: async (css: string) => {
            await DataStore.set("NavinecordQuickCss", css);
        ...
        async openEditor() {
            ...
            const win = open("about:blank", "NavinecordQuickCss", features);
            if (!win) {
            ...
            win.getCurrentCss = () => NavinecordNative.quickCss.get();
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("NavinecordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("NavinecordSettings", JSON.stringify(s)),
