...
While it was primarily created for Navinecord, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @Navinecord/discord-types
yarn add -D @Navinecord/discord-types
pnpm add -D @Navinecord/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@Navinecord/discord-types";
...
```ts
import { ApplicationCommandType } from "@Navinecord/discord-types/enums";
