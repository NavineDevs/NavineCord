...
const commonOptions = {
    ...commonOpts,
    entryPoints: ["browser/Navinecord.ts"],
    format: "iife",
...
const buildConfigs = [
    ...
    },
    {
        ...
        footer: { js: "//# sourceURL=file:///NavinecordWeb" }
    },
    {
        ...commonOptions,
    ...
    },
    {
        ...
        outfile: "dist/Navinecord.user.js",
        banner: {
            js: readFileSync("browser/userscript.meta.js", "utf-8").replace("%version%", `${VERSION}.${new Date().getTime()}`)
        },
        footer: {
            // UserScripts get wrapped in an iife, so define Navinecord prop on window that returns our local
            js: "Object.defineProperty(unsafeWindow,'Navinecord',{get:()=>Navinecord});"
        }
    }
];
...
async function buildExtension(target, files) {
    const entries = {
        "dist/Navinecord.js": await readFile("dist/extension.js"),
        "dist/Navinecord.css": await readFile("dist/extension.css"),
...
const appendCssRuntime = readFile("dist/Navinecord.user.css", "utf-8").then(content => {
    const cssRuntime = `unsafeWindow._vcUserScriptRendererCss=\`${content.replaceAll("`", "\\`")}\``;

    return appendFile("dist/Navinecord.user.js", cssRuntime);
});
