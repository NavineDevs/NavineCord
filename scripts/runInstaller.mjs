...
const BASE_URL = "https://github.com/Navinecord/Installer/releases/latest/download/";
const INSTALLER_PATH_DARWIN = "NavinecordInstaller.app/Contents/MacOS/NavinecordInstaller";
...
function getFilename() {
    switch (process.platform) {
        case "win32":
            return "NavinecordInstallerCli.exe";
        case "darwin":
            return "NavinecordInstaller.MacOS.zip";
        case "linux":
            return "NavinecordInstallerCli-linux";
        default:
...
async function ensureBinary() {
    ...
    const outputFile = process.platform === "darwin"
        ? join(FILE_DIR, "NavinecordInstaller")
        : downloadName;
    ...
    const res = await fetch(BASE_URL + filename, {
        headers: {
            "User-Agent": "Navinecord (https://github.com/Vendicated/Navinecord)",
            "If-None-Match": etag
        }
    });
    ...
    if (process.platform === "darwin") {
        ...
        logAndRun(`sudo spctl --add '${outputFile}' --label "Navinecord Installer"`);
...
try {
    execFileSync(installerBin, args, {
        ...
        env: {
            ...process.env,
            Navinecord_USER_DATA_DIR: BASE_DIR,
            Navinecord_DEV_INSTALL: "1"
        }
    });
} catch {
